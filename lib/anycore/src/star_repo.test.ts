...
import {starRepositoryCloneFactoryCtor, starRepositoryServerFactoryCtor, stateModifierFunctionType} from './star_repo';
...
describe('starRepo', () => {
  const starRepositoryServerFactory = starRepositoryServerFactoryCtor(tuidFactory);
  ...
  it('multi-client example', () => {
    ...
    assert.strictEqual(srClient0.eventRegistrySignature, srServer.eventRegistrySignature);
    ...
    assert(c2cm1);
    const slcm0 = srServer.localCommit('append', {z: 9});
    ...
    // slcm0 sent to each client
    srClient0.onRebase(slcm0);
    srClient1.onRebase(slcm0);
    srClient2.onRebase(slcm0);
    assert.deepStrictEqual(srClient0.localState, {z: 9, a: 1});
    assert.deepStrictEqual(srClient0.serverState, {z: 9});
    assert.deepStrictEqual(srClient1.localState, {z: 9, b: 2});
    assert.deepStrictEqual(srClient1.serverState, {z: 9});
    assert.deepStrictEqual(srClient2.localState, {z: 9});
    assert.deepStrictEqual(srClient2.serverState, {z: 9});
    ...
    assert.deepStrictEqual(srClient0.localState, {z: 9, a: 1, c: 3});
    assert.deepStrictEqual(srClient0.serverState, {z: 9, c: 3});
    assert.deepStrictEqual(srClient1.localState, {z: 9, b: 2, c: 3});
    assert.deepStrictEqual(srClient1.serverState, {z: 9, c: 3});
    assert.deepStrictEqual(srClient2.localState, {z: 9,});
    assert.deepStrictEqual(srClient2.serverState, {z: 9, c: 3});
    ...
    assert.deepStrictEqual(srClient0.localState, {z: 9, a: 1, c: 3});
    assert.deepStrictEqual(srClient0.serverState, {z: 9, a: 1, c: 3});
    assert.deepStrictEqual(srClient1.localState, {z: 9, a: 1, b: 2, c: 3});
    assert.deepStrictEqual(srClient1.serverState, {z: 9, a: 1, c: 3});
    assert.deepStrictEqual(srClient2.localState, {z: 9, a: 1});
    assert.deepStrictEqual(srClient2.serverState, {z: 9, a: 1, c: 3});
    ...
    assert.deepStrictEqual(srClient0.localState, {z: 9, b: 2, a: 1, c: 3});
    assert.deepStrictEqual(srClient0.serverState, {z: 9, b: 2, a: 1, c: 3});
    assert.deepStrictEqual(srClient1.localState, {z: 9, a: 1, b: 2, c: 3});
    assert.deepStrictEqual(srClient1.serverState, {z: 9, b: 2, a: 1, c: 3});
    assert.deepStrictEqual(srClient2.localState, {z: 9, b: 2, a: 1});
    assert.deepStrictEqual(srClient2.serverState, {z: 9, b: 2, a: 1, c: 3});
    ...
    assert.deepStrictEqual(srClient0.localState, {z: 9, b: 2, a: 1});
    assert.deepStrictEqual(srClient0.serverState, {z: 9, b: 2, a: 1});
    assert.deepStrictEqual(srClient1.localState, {z: 9, a: 1, b: 2});
    assert.deepStrictEqual(srClient1.serverState, {z: 9, b: 2, a: 1});
    assert.deepStrictEqual(srClient2.localState, {z: 9, b: 2, a: 1});
    assert.deepStrictEqual(srClient2.serverState, {z: 9, b: 2, a: 1});

    assert.strictEqual(srClient1.serverStateSignature, '4f0e85ed89ff4ada');
    srClient1.serverState = {};
    assert.strictEqual(srClient1.serverStateSignature, '2e1472b57af294d1');
    assert.strictEqual(srClient1.clientId, '00000000000000000000000000000003')

    assert.strictEqual(0, errors.length);

    const eventRegistrySignature = '5ed83c298a4eaafc';
    const preMergeSignature = '4f0e85ed89ff4ada';
    ...
    assert.deepStrictEqual(srServer.state, {z: 9, a: 1, b: 2});
    assert.strictEqual(srServer.stateSignature, "4f0e85ed89ff4ada");
